...
Kromě toho najdete u každého tématu (s výjimkou úvodní hodiny) Základní a případně i Rozšiřující přípravu na každou přednášku. Základní příprava je povinná v tom smyslu, že ve výkladu se počítá s tím, že ji studující před přednáškou absolvoval. Koncepty, které jsou obsaženy v Základní přípravě, už na přednášce nebudou detailně vysvětlovány. Naopak budu počítat s tím, že už jste si je osvojili a budu na ně navazovat. Základní příprava je přibližně na 20 až 60 minut na každou přednášku, prosím, opravdu si na ni čas vyhraďte. Otázky v závěrečném testu také předpokládají, že jste základní přípravu absolovovali. Rozšiřující příprava je nabídkou, jak se k tématu dozvědět předem něco víc a usnadnit si tak orientaci v přednášené látce. S výkladem se buď do určité míry překrývá, nebo přináší doplňkovou perspektivu.   
...
-   *Cíle výuky:* Cílem je seznámit studující s různými typy proměnných. Pozornost je poté věnována kategorickým proměnným a různým způsobům jejich prezentace včetně prezentace vizuální. Speciální pozornost je věnována binárním / dichotomickým datům a problematice absolutního a relativního rizika.
...
-   *Cíle výuky:* Cílem je představit studujícím koncept empirické distribuce dat (empirického rozdělení dat) a možnosti jejího grafického a numerické popisu s důrazem na míry centrality.

-   *Výsledky učení - studující dokáže:*

    -   Definovat a ve správných souvislostech použít pojmy: empirická distribuce dat, boxplot (krabicový graf), histogram, aritmetický průměr, medián, modus, kvartil, percentil.
...
-   *Cíle výuky:* Přednáška rozšiřuje výklad o konceptu empirické distribuce dat, tentokrát s důrazem na míry variability. V rámci přednášky je také představena problematika exponenciální funkce (exponenciálního růstu) a možnosti využití logaritmického měřítka pro práci s (velmi) šikmými daty. Studenti jsou také seznámeni problematikou standardizace a z-skórů. V rámci přednášky je též představeno uniformní a normální rozdělení.

-   *Výsledky učení - studující dokáže:*

    -   Definovat a ve správných souvislostech použít pojmy: exponenciální funkce, logaritmické měřítko, logaritmická transformace, rozsah, percentil, kvartil, mezikvartilové rozpětí, směrodatná odchylka, rozptyl, Giniho koeficient, šikmost, špičatost, uniformní rozdělení, normální rozdělení.
