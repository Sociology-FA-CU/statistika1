...
* Všimněte si: čím vyšší je PDF, tím strmější je (obecná vlastnost funkcí rozdělení pravděpodobnosti, nejen pro normální rozdělení)
...
>- Kolik procent pozorování zeleného rozdělení spadá mezi hodnoty -5 a 1?
>- Kolik procent pozorování červeného rozdělení spadá mezi hodnoty -1 a 2?
...
1) Jaké rozdělení má proměnná X?
1) Pr(Y=0), když $p_X$ = 0.2
1) Pr(Y=2), když $p_X$ = 0.2
1) Pr(Y>=2), když $p_X$ = 0.2
1) E(Y)?, když $p_X$ = 0.2
1) E(Y)?, když $p_X$ = 0.5
1) Var(Y)?, když $p_X$ = 0.4
1) Které stavy byste označili za běžné a které za extrémní?
1) Pokud byste předpokládali $p_X$ = 0.2, je 8 příznivců hybridní výuku extrémní stav, který by zpochybnil Vás předpoklad?
1) Jak se výsledky změní, pokud n = 1000? Jak se změní tvar rozdělení?


::: {.notes}
**1** Alternativní čili Bernoulliho. **2** $0.8^10 = 0.107$. **3** Pozor, toto není $0.2^2 * 0.8^8$, protože 2 z 8 lze vybrat mnoha způsoby. Je potřeba použít kombinatoriku, resp. vzorec výše. **4** Nejlepší je spočítat Pr(Y=0) a Pr(Y=1) a jejich součet odečíst od 1.  
:::
...
Klasický přístup k inferenci: statistika nám pomáhá určit, jak (ne)pravděpodobná jsou určitá data za předpokladu určitého modelu. 
